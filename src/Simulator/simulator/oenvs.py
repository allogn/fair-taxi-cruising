import random, time
import logging
from src.Simulator.simulator.oobjects import *
from src.Simulator.simulator.outilities import *
...
class CityReal:
    ...
    def __init__(self, mapped_matrix_int, order_num_dist, idle_driver_dist_time, idle_driver_location_mat, order_time_dist, order_price_dist,
                 ...
                 global_flag="", time_interval=10):
        """
        driver management: @alvis
            onoff:
                if there is onoff driver location : if there are more drivers to add, then they are added
                proportionally to the difference between current distribution and idle_driver_location_mat
            bootstrap:
                if global_flag=global, then number of drivers are decided according to idle_driver_dist_time (??? once ???)
                and then added like in onoff procedure
                if glocal_flag!=global, then number of drivers is equal to the sum of idle_driver_location_mat
                and then also distributed randomly proportionally to the difference between idle_driver_location_mat and current distribution.
                new drivers are first pulled from those who went offline, then new are added if necessary

                idle_driver_dist_time seems to be used only when bootstrapping.
                idle_* matrices for non-zero time should be filled only if onoff is non-zero, or reset_clean is for non-zero
        ...
        self.n_intervals = 1440 // time_interval # @alvis
        ...
        assert l_max <= M-1 and l_max <= N-1, "Maybe too small n, should be larger than 9"
        ...
        if order_time_dist != None: # my addition
    ...
    def reset_clean(self, generate_order=1, ratio=1, city_time=""):
        ...
        # if generate_order == 1: @alvis -- otherwise real_orders are not loaded
    ...
    def step_driver_online_offline_control_new(self, n_idle_drivers): # called from reset_clean
        """ control the online offline status of drivers
        :param n_idle_drivers: the number of idle drivers expected at current moment
        :return:
        """
        assert self.city_time == 0, "Assume this is called only when initializing environment"

        # modified - simply assign drivers
        curr_idle_driver_distribution = self.idle_driver_location_mat[0, :]
        self.drivers = {}
        self.n_drivers = 0
        for node_id in range(len(self.nodes)):
            self.nodes[node_id].drivers = {}
            for i in range(int(curr_idle_driver_distribution[node_id])):
                n_total_drivers = len(self.drivers.keys())
                added_driver_id = n_total_drivers
                self.drivers[added_driver_id] = Driver(added_driver_id)
                self.drivers[added_driver_id].set_position(self.nodes[node_id])
