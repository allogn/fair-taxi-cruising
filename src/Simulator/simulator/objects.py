...
class Node(object):
    ...
    def generate_order_real(self, l_max, order_time_dist, order_price_dist, city_time, nodes, seed):
        ...
        for ii in np.arange(num_order_t):
            ...
            self.orders.append(Order(nodes[current_node_id],
                                     nodes[np.random.choice(destination_node, 1)[0]], #randomly select a node out of those located at the duration
                                     city_time,
                                     duration,
                                     price, 1))
        return
    ...
    def remove_idle_driver_random(self):
        ...
        removed_driver_id = np.random.choice([d for d, item in self.drivers.items() if item.onservice is False and item.online is True])
        self.remove_driver(removed_driver_id)
    ...
    # this seems deprecated and using the next "real" instead
    ...
    def simple_order_assign_real(self, city_time, city, wc, maxincome = None):
        '''
        This convers additional case when a customer takes a car outside a region (then goes offline).
        A driver is offline if it does not take orders anymore, outside city for example
        '''
        reward = 0
        num_assigned_order = min(self.order_num, self.idle_driver_num) # this for more general case

        # some cars may stay where they are

        # if self.order_num < self.idle_driver_num:
        #     raise Exception("Dispatching actions (orders) for my algorithms ({}) should always be gequal to # idle cars ({})".format(self.order_num, self.idle_driver_num))

        served_order_index = []

        init_idle = self.idle_driver_num
        init_driver_num = len(self.drivers)

        random_drivers = np.random.permutation([assigned_driver for key, assigned_driver in self.drivers.items()
                                                    if assigned_driver.onservice is False and assigned_driver.online])
        assert(len(random_drivers) == self.idle_driver_num)

        if len(self.orders) > 0:
            random_customers = np.random.choice(len(self.orders),num_assigned_order,replace=False)
            i = 0
            for order_id in random_customers:
                order_to_serve = self.orders[order_id]
                order_to_serve.set_assigned_time(city_time)
                self.order_num -= 1
                served_order_index.append(order_id)
                assigned_driver = random_drivers[i]

                old_income = assigned_driver.reward
                assigned_driver.take_order(order_to_serve)
                new_income = assigned_driver.reward

                assert(new_income - old_income - order_to_serve.get_price() < 0.00001)
                # reward += order_to_serve.get_price()
                reward = min([new_income, maxincome]) - min([old_income, maxincome])

                # print("driver {} takes order with price {}".format(assigned_driver._driver_id, order_to_serve._p))
                removed_driver = self.drivers.pop(assigned_driver.get_driver_id(), None)
                assert removed_driver is not None
                city.n_drivers -= 1
                i += 1
        ...
        # print("Node {}: {} were assigned to orders, revenue {}.".format(self._index, num_assigned_order, reward))
...
class Driver(object):
    __slots__ = ("online", "onservice", 'order', 'node', 'city_time', '_driver_id', 'reward')
