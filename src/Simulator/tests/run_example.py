...
def running_example():
    mapped_matrix_int = np.array([[1, -100, 3], [5, 4, 2], [6, 7, 8]]) # indexes of cells, -100 means not valid. counts from 1 to 8 (num_valid_grid)
    M, N = mapped_matrix_int.shape
    order_num_dist = []
    num_valid_grid = 8 # number of active cells
    ...
    idle_driver_dist_time = [[10, 1] for _ in np.arange(144)] # dist = distribution

    n_side = 6
    l_max = 2 # max duration of an order
    order_time = [0.2, 0.2, 0.15,
                  0.15,  0.1,  0.1,
                  0.05, 0.04,  0.01] # distribution of length: sums to 1
    ...
    for tt in np.arange(144):
        order_real += [[1, 5, tt, 1, 13.2], [2, 6, tt, 1, 13.2], [3, 1, tt, 1, 13.2],
                       ...
                       [9, 10, tt, 1, 13.2], [10, 6, tt, 1, 13.2], [9, 7, tt, 1, 13.2]] # origin grid, destination grid, start time, end time, price
        onoff_driver_location_mat.append([[-0.625,       2.92350389], # mean and std of online driver number - offline driver number
